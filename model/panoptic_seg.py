# https://github.com/dvlab-research/PanopticFCN/blob/main/panopticfcn/panoptic_seg.py
import torch
...
from .backbone_utils import build_backbone, build_semanticfpn
from .head import (
    build_position_head,
    build_feature_encoder,
    build_kernel_head,
    build_stuff_generator,
    build_thing_generator,
)
from .utils import multi_apply, topk_score
...
class PanopticFCN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        datasest_names = self.cfg.DATASETS.TRAIN
        self.meta = MetadataCatalog.get(datasest_names[0])
    ...
    def forward(self, batched_inputs):
        ...
        * Other information that's included in the original dicts, such as:
        ...
        features_in = [
            features[_feat] for _feat in self.in_feature
        ]  # ["p3", "p4", "p5", "p6", "p7"]
        pred_centers, pred_regions, pred_weights = multi_apply(
            self.forward_single_level, features_in
        )  # [8, H/8, W/8], [19, H/8, W/8], [256, H/8, W/8] - 1/8, 1/16, 1/32, 1/64, 1/128
        ...
        if self.training:
            ...
            # return gt_dict, pred_centers, pred_regions, pred_weights
            return self.losses(
                pred_centers, pred_regions, pred_weights, encode_feat, gt_dict
            )
        else:
    ...
    def losses(self, pred_centers, pred_regions, pred_weights, encode_feat, gt_dict):
        ...
        encode_feat = encode_feat.reshape(*feat_shape[:2], -1)  # [B, C, H*W/16]
        (
            loss_pos_ths,
            loss_pos_sts,
            idx_feat_th,
        ...
        # for thing
        thing_gt_idx = [
            _gt[:, : thing_nums[_idx]] for _idx, _gt in enumerate(gt_dict["index_mask"])
        ]
        thing_gt_idx = torch.cat(thing_gt_idx, dim=1)
        thing_gt_idx = thing_gt_idx.reshape(-1).bool()
        thing_gt_num = int(thing_gt_idx.sum())
        thing_gt = [
            _gt[:, : thing_nums[_idx], ...] for _idx, _gt in enumerate(gt_dict["inst"])
        ]
        thing_gt = torch.cat(thing_gt, dim=1)
        loss_thing = weighted_dice_loss(
            thing_pred,
            thing_gt,
            gt_num=thing_gt_num,
        ...
        # for stuff
        stuff_gt_idx = [
            _gt[:, : stuff_nums[_idx]] for _idx, _gt in enumerate(gt_dict["sem_index"])
        ]
        ...
        # position loss
        loss["loss_pos_th"] = (
            self.pos_weight_thing * sum(loss_pos_ths) / max(thing_gt_num, 1)
        )
        loss["loss_pos_st"] = (
            self.pos_weight_stuff * sum(loss_pos_sts) / max(feat_shape[0], 1)
        )
        # segmentation loss
    ...
    def loss_single_level(
        ...
        loss_pos_th = sigmoid_focal_loss(
            pred_center,
            gt_center,
            mode="thing",
        ...
        loss_pos_st = sigmoid_focal_loss(
            pred_region,
            gt_sem_scores,
            mode="stuff",
            alpha=self.focal_loss_alpha,
        ...
        guided_inst = F.interpolate(
            gt_inst, size=(feat_h, feat_w), mode="bilinear", align_corners=False
        )
        ...
        weighted_values, guided_index = torch.topk(
            guidence.reshape(*guided_inst.shape[:2], -1), k=self.weighted_num, dim=-1
        )
        ...
        num_class_st = torch.clamp(
            gt_sem_masks.reshape(-1, 1, *weight_shape[-2:]).sum(
    ...
    def inference_single_level(self, pred_center, pred_region, pred_weights, pool_size):
        ...
        center_pool = F.avg_pool2d(
            pred_center, kernel_size=pool_size, stride=1, padding=(pool_size - 1) // 2
        )
        ...
        sub_score, sub_index, sub_class, ys, xs = topk_score(
            pred_center, K=top_num, score_shape=center_shape
        )
    ...
    def inference(
        ...
        for img_idx, result_img in enumerate(results):
            ...
            if thing_num == 0:
                result_instance = Instances(
                    ori_shape, pred_masks=[], pred_boxes=[], pred_classes=[], scores=[]
                )
            else:
            ...
            pred_stuff = F.interpolate(
                pred_stuff, size=ori_shape, mode="bilinear", align_corners=False
            )[0]
    ...
    def process_inst(self, classes, scores, pred_inst, img_shape, ori_shape):
        ...
        pred_inst = F.interpolate(
            pred_inst.unsqueeze(0),
            scale_factor=self.common_stride,
            mode="bilinear",
            align_corners=False,
        )[..., : img_shape[0], : img_shape[1]]
        pred_inst = F.interpolate(
            pred_inst, size=ori_shape, mode="bilinear", align_corners=False
        )[0]
    ...
    def combine_thing_and_stuff(
        ...
        if thing_cate is not None:
            ...
            if keep.sum() > 0:
                ...
                for _idx, (_mask, _cate, _score) in enumerate(
                    zip(pred_thing, thing_cate, thing_score)
                ):
                    ...
                    if (
                        mask_area == 0
                        or intersect_area * 1.0 / mask_area > overlap_threshold
                    ):
                        continue
                    if intersect_area > 0:
                    ...
                    category_id = self.meta.thing_train_id2contiguous_id[
                        thing_category_id
                    ]
