...
class GenerateGT(object):
    ...
    def assign_multi_scale(
        self, gt_instances, gt_semantics, image_shape, inst_outsize, feat_shape
    ):
        """
        ...
        h, w = gt_sem.shape[-2:]
        new_h, new_w = int(h / self.common_stride + 0.5), int(
            w / self.common_stride + 0.5
        )
        if 0:  # self.depth_on:
            gt_sem = F.interpolate(
                gt_sem.unsqueeze(0), size=(new_h, new_w), mode="nearest"
            )[0]
        else:
            gt_sem = F.interpolate(
                gt_sem.unsqueeze(0),
        ...
        if hasattr(gt_instances, "gt_masks"):
            ...
            bit_masks, mass_centers, scales = GenerateGT.decode_bitwise_mask(
            ...
            for layer_idx in range(len(feat_shape)):
                ...
                gt_inst_per = self.ignore_val * torch.ones(
                    self.tensor_dim, *inst_outsize, device=self.device
                )
    ...
    def _label_assignment(
        ...
        gt_instance = self.ignore_val * torch.ones(
            self.tensor_dim, *inst_outsize, device=self.device
        )
        ...
        box_rescale = [
            feat_shape[-2] / image_shape[-2],
            feat_shape[-1] / image_shape[-1],
        ]
        center_rescale = [
            feat_shape[-2] / inst_outsize[-2],
    ...
    def _sem_assign(self, gt_semantic, feat_shape):
        ...
        if 0:  # self.depth_on:
            gt_scoremap = F.interpolate(
                gt_semantic.unsqueeze(0), size=feat_shape, mode="nearest"
            )[0]
        else:
            gt_scoremap = F.interpolate(
    ...
    def generate(self, batched_input, images, features, inst_feat, test_only=False):
        ...
        (
            ...
            self.assign_multi_scale,
            gt_instances,
            gt_semantics,
            image_shape=image_shape,
            inst_outsize=inst_shape,
    ...
    @staticmethod
    def decode_bitwise_mask(
        common_stride, bit_masks, device, use_mass=False, sampling_strategy="bilinear"
    ):
        """
        ...
        mass_centers = []
        scale = 1.0 / common_stride
        h, w = bit_masks.shape[-2:]
        new_h, new_w = int(h * scale + 0.5), int(w * scale + 0.5)
        if sampling_strategy == "bilinear":
            bit_masks = F.interpolate(
                bit_masks.float().unsqueeze(0),
                size=(new_h, new_w),
                mode="bilinear",
                align_corners=False,
            )[0]
            ...
            bit_masks = F.interpolate(
                bit_masks.float().unsqueeze(0), size=(new_h, new_w), mode="nearest"
            )[0]
        else:
    ...
    def generate_score_map(
        fmap, gt_class, gt_wh, centers_int, min_overlap, sigma_factor=6, device=None
    ):
        """
