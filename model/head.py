...
class PositionHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.position_head = SingleHead(
            in_channel + 2 if coord else in_channel,
            conv_dims,
            num_convs,
            deform=deform,
            coord=coord,
...
class ThingGenerator(nn.Module):
    ...
    def forward(
        self, x, feat_shape, idx_feat, idx_shape, pred_cate=None, pred_score=None
    ):
        n, c, h, w = feat_shape
        if idx_shape > 0:
    ...
    def cal_similarity(self, base_w, anchor_w, sim_type="cosine"):
        if sim_type == "cosine":
            a_n, b_n = base_w.norm(dim=1).unsqueeze(-1), anchor_w.norm(dim=1).unsqueeze(
                -1
            )
            ...
            similarity = 1.0 - (base_w - anchor_w).abs().clamp(min=1e-6).norm(dim=1)
        else:
...
class StuffGenerator(nn.Module):
    ...
    def forward(
        self, x, feat_shape, idx_feat, idx_mask, pred_cate=None, pred_score=None
    ):
        n, c, h, w = feat_shape
        meta_weight = self.embed_extractor(idx_feat)
...
#             if not self.training:
#                 meta_weight, pred_cate, pred_score, pred_depth = self.kernel_fusion(
#                     meta_weight, pred_cate, pred_score, pred_depth
#                 )
#             inst_pred = torch.matmul(meta_weight, x)
#             inst_pred = inst_pred.reshape(n, -1, h, w)
...
#         pred_cate=None,
#         pred_score=None,
#         pred_depth_kernel=None,
#     ):
#         n, c, h, w = feat_shape
#         meta_weight = self.embed_extractor(idx_feat)
#         meta_weight = meta_weight.reshape(n, -1, self.conv_dims)
#         if not self.training:
#             meta_weight, pred_cate, pred_score, pred_depth_kernel = self.kernel_fusion(
...
#             pred_depth_kernel = (
#                 torch.mm(label_matrix, pred_depth_kernel.squeeze(-1).squeeze(-1))
#                 / label_norm
#             )
