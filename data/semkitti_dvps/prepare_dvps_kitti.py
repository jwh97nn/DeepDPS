...
root = "/home/junwen/Datasets/SemKITTI-DVPS/video_sequence/"

for split in ["val", "train"]:
    imgs = glob(root + split + "/*_leftImg8bit.png")
    seg_gts = glob(root + split + "/*_gtFine_class.png")
    depth_gts = glob(root + split + "/*_depth_*.png")
    assert len(imgs) == len(seg_gts) == len(depth_gts) > 0, (
        len(imgs),
        len(seg_gts),
        len(depth_gts),
    )
    ...
    prefix2 = seg_gts[0][: -len("_gtFine_class.png")]
    # prefix3 = depth_gts[0][:-len('_depth.png')]
    prefix3 = depth_gts[0][:13]
    ...
    save_dir = os.path.join(root, "dvps_kitti_{}.json".format(split))
