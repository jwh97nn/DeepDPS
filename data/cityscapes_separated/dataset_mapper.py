...
class CityscapesPanopticDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        stuff_classes = np.array(
            list(self.meta.stuff_dataset_id_to_contiguous_id.values())
        )

        # USER: Remove if you don't do semantic/panoptic segmentation.
        if "pan_seg_file_name" in dataset_dict:
            # pan_seg_gt = utils.read_image(dataset_dict.pop("pan_seg_file_name"))
            pan_seg_gt = utils.read_image(dataset_dict["pan_seg_file_name"])
            pan_seg_gt = (
                pan_seg_gt[:, :, 0]
                + 256 * pan_seg_gt[:, :, 1]
                + 256 * 256 * pan_seg_gt[:, :, 2]
            )
        else:
            raise NotImplementedError(
                "Currently only possible if pan seg GT image file name is given"
            )
            # pan_seg_gt = None
        ...
        for segment in dataset_dict["segments_info"]:
            if segment["category_id"] in things_classes:
                ...
                annotation["category_id"] = self.meta.contiguous_id_to_thing_train_id[
                    segment["category_id"]
                ]
        ...
        if "sem_seg_file_name" in dataset_dict:
            sem_seg_gt_tmp = utils.read_image(
                dataset_dict.pop("sem_seg_file_name"), "L"
            ).squeeze(2)
        else:
            raise NotImplementedError(
                "Currently only possible if sem seg GT image file name is given"
            )
            # sem_seg_gt = None
        ...
        if self.cfg.MODEL.POSITION_HEAD.STUFF.ALL_CLASSES:
            sem_seg_gt = np.where(
                np.isin(sem_seg_gt_tmp, stuff_classes),
                sem_seg_gt_tmp,
                self.meta.ignore_label,
            )
        else:
            if self.cfg.MODEL.POSITION_HEAD.STUFF.WITH_THING:
                ...
                sem_seg_gt = np.where(
                    np.isin(sem_seg_gt_tmp, np.array(things_classes)), 0, sem_seg_gt
                )
            else:
                sem_seg_gt = np.where(
                    np.isin(sem_seg_gt_tmp, stuff_classes),
...
if __name__ == "__main__":
    ...
    # from data.cityscapes.cityscapes_panoptic_separated import (
    #     register_all_cityscapes_panoptic_separated,
    # )
    ...
    # register_all_cityscapes_panoptic_separated(root="/home/junwen/Datasets/")
    ...
    for batched_inputs in d:
        ...
        cv2.imwrite(
            "img.png", batched_inputs[0]["image"].permute(1, 2, 0).numpy()[:, :, ::-1]
        )

        img = batched_inputs[0]["image"].permute(1, 2, 0).numpy()[:, :, ::-1]
        metadata = MetadataCatalog.get(cfg.DATASETS.TRAIN[0])
        visualizer = Visualizer(img, metadata=metadata)

        print(batched_inputs[0]["sem_seg"])
        sem_vis = visualizer.draw_panoptic_seg(
            batched_inputs[0]["sem_seg"], batched_inputs[0]["segments_info"]
        )

        visualizer.get_output().save("img1.png")
