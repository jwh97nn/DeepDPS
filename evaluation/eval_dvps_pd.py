...
def vpq_eval(element):
    ...
    # Convert gt (255 -> ign_id, [0-10] -> *= 1000)
    gt_ids[gt_ids == 255] = ign_id * max_ins
    for id in np.unique(gt_ids):
        if id <= 10:
            gt_ids[gt_ids == id] *= max_ins
    # Convert pred
    pred_ids[pred_ids == 255] = ign_id * max_ins
    for id in np.unique(pred_ids):
        if id <= 10:
            pred_ids[pred_ids == id] *= max_ins
