...
def vpq_eval(element):
    ...
    # ign_ids = {gt_id for gt_id in six.iterkeys(gt_areas) if gt_id == 255}
    print(f"ign_ids", ign_ids)
    ...
    # print([iou_per_class, tp_per_class, fn_per_class, fp_per_class])
    # assert 0
...
def eval(element):
    ...
    # preds = [np.array(Image.open(os.path.join(pred_dir, pred))) for pred in preds]

    # cats = [pred[:, :, 0] for pred in preds]
    # inds = [pred[:, :, 1] for pred in preds]
    # preds = [
    #     cat.astype(np.uint32) * max_ins + ind.astype(np.uint32)
    #     for cat, ind in zip(cats, inds)
    # ]

    # gts = [np.array(Image.open(os.path.join(gt_dir, name))) for name in gts]
    gts = [
        np.array(Image.open(os.path.join(gt_dir, name)))
        for name in gts
        if "_instanceTrainIds.png" in name
    ]

    abs_rel = 0
    if depth_thres > 0:
        # depth_preds = [
        #     np.array(Image.open(os.path.join(depth_dir, name))) for name in depth_preds
        # ]
        depth_preds = [
        ...
        abs_rel = np.mean(
            np.abs(depth_pred_cat[depth_mask] - depth_gt_cat[depth_mask])
...
def main(eval_frames, pred_dir, gt_dir, depth_thres):
    ...
    depth_gt_names = [name.name for name in depth_gt_names if "depth.png" in name.name]
    ...
    for seq_id in gts:
        ...
        # all_files.append(
        #     [preds[seq_id], gts[seq_id], depth_preds_seq_id, depth_gts[seq_id]]
        # )
        all_files.append(
            [
                # preds[seq_id],
    ...
    print(
        ...
        "PQ     PQ_th  PQ_st\n",
        "{:.2f}  {:.2f}  {:.2f}".format(
            pq.mean() * 100, pq[11:].mean() * 100, pq[:11].mean() * 100
        ),
    )
    ret = dict()
...
if __name__ == "__main__":
    ...
    # pred_dir = args.pan_dir
    # depth_thres = args.depth_thres
    # gt_dir = "/hjw/Datasets/Cityscapes-DVPS/video_sequence/val"

    # main(eval_frames, pred_dir, gt_dir, depth_thres)

    from panopticapi.utils import id2rgb, rgb2id

    gt = "/hjw/Datasets/Cityscapes/gtFine/cityscapes_panoptic_val/frankfurt_000000_000294_gtFine_panoptic.png"
    ...
    # fmt: on
    gt_after = np.ones_like(gt) * 255
    for k, v in gt_dict.items():
        gt_after[gt == k] = v
    # print(f"pred_after {np.unique(gt_after)}\n")

    pred = "output/test1/inference/frankfurt_000000_000294_leftImg8bit.png"
    pred = np.array(Image.open(pred))
    pred = rgb2id(pred)
    # print(f"pred {np.unique(pred)}\n")
    pred_after = np.ones_like(pred) * 255
    # fmt: off
    ...
    for k, v in pred_dict.items():
        pred_after[pred == k] = v
    # print(f"pred_after {np.unique(pred_after)}\n")

    results = [vpq_eval(([pred_after], [gt_after]))]
    # print(results)
